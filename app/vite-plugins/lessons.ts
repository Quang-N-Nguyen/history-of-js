...
function buildLesson(pain: number): Lesson {
  ...
  const exercisesRoot = join(chapterDir, 'exercises');
  const solutionsRoot = join(chapterDir, 'solutions');
  const docPath = join(REPO_ROOT, 'docs', 'pains', `${id}.md`);
...
export default lessons;`;
    ...
    configureServer(server) {
      ...
      server.watcher.add(join(REPO_ROOT, 'docs', 'pains'));
    },
  };
}
